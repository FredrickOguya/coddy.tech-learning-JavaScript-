...
The length is the easiest to check for now, so that is what you should check first. Check if the length is 15 or 16 using a while loop and dividing the number by 10 each iteration and rounding it down. If it is not 15 or 16, print Invalid and stop the program using return.

Hint 2
Revealed

If you read Luhn's Algorithm correctly, we are calculating two sums while looping through the number. The first sum sum1 is the sum of every other digit multiplied by 2, starting with the second to last digit. Keep in mind that if you multiply a digit by 2 and the result is bigger than 10, you need to add the digits of that number too, not the actual number. The second sum sum2 is the sum of all remaining numbers, aka every other number starting from the last number. So, we can calculate both of these sums at the same time. Using a do - while loop that has a condition of cardNumber > 0, we can remove the last digit and add it to sum2, then remove the second to last number, multiply it by 2 and add it to sum1. Add sum1 and sum2 together. If total % 10 !== 0, print INVALID and stop the program.
